...
new VKVideoMobileController(BaseController, Styles, TimeUtil, UIUtil);

function VKVideoMobileController(
                    baseController,
                    style, timeUtil, uiUtil) {
    ...
    function waitForProgressBarContainerElement() {
        return new Promise((res, rej) => {
            ...
            waitForElement(shadowEl).then(() => {
                ...
                waitForElement('.video-container', shadowDoc).then(() => (
                    shadowDoc.querySelector('.video-container', shadowDoc).addEventListener('click', () => {
    ...
    // START HERE:
    // Wait for the video click bar to initalize
    waitForProgressBarContainerElement().then(init);
    
    function init() {

        // var SEL_VIDEO_PLAY_BUTTON = '.ytp-play-button';
        var SEL_BELOW_VIDEO = '.VideoPage__playerContainer'; //'.vkitInternalHeaderLayout';
    ...
    function getServiceIdAndVideoIdAndURL(time=0) {
        ...
        try {
            ...
            const hasVKMobileVideoQuery = window.location.href.match(/.*m\.vkvideo\.ru\/video([^\/]*)/);

            if (!!hasVKVideo && (!!hasVKVideoQuery || !!hasVKMobileVideoQuery)) {
                serviceId = 'vkvideo';
                videoId = !!hasVKVideoQuery? hasVKVideoQuery[1]:hasVKMobileVideoQuery[1];
